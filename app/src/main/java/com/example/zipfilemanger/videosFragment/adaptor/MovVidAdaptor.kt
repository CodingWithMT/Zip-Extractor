...
var movLongClick : ((Int) -> Unit)? = null
var movItemClick : ((Int) -> Unit)? = null

class MovVidAdaptor(val context: Context, var movVidList: ArrayList<GalleryEnt>) :
    RecyclerView.Adapter<MovVidAdaptor.ViewHolder>() {

    private var selectedMovVidList: ArrayList<GalleryEnt> = ArrayList()
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val selectFolder = movVidList[position]
        ...
        if (movVidList[position].isChecked) {
    ...
    override fun getItemCount(): Int {
        return movVidList.size
    }

    fun getAllSelected() : ArrayList<GalleryEnt> {
        selectedMovVidList = ArrayList()
        for (his in movVidList) {
            if (his.isChecked) {
                selectedMovVidList.add(his)
            }
        }
        return selectedMovVidList

    }

    fun setChecked(position: Int) {
        movVidList[position].isChecked = !movVidList[position].isChecked
        notifyItemChanged(position)
    }

    fun setData(list: List<GalleryEnt>) {
        movVidList = list as ArrayList<GalleryEnt>
        notifyDataSetChanged()
    }

    fun updateList(data: List<GalleryEnt>) {
        movVidList.clear()
        movVidList.addAll(data)
        notifyDataSetChanged()
    }
}