...
var allVidLongClick : ((Int) -> Unit)? = null
var allVidItemClick : ((Int) -> Unit)? = null

class AllVidAdaptor(val context: Context, var vidList: ArrayList<GalleryEnt>) :
    RecyclerView.Adapter<AllVidAdaptor.ViewHolder>() {

    private var selectedAllVidList: ArrayList<GalleryEnt> = ArrayList()
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val selectFolder = vidList[position]
        ...
        holder.itemView.setOnLongClickListener {
            allVidLongClick?.invoke(position)
            true
        }
        holder.itemView.setOnClickListener {
            allVidItemClick?.invoke(position)
        }

        if (vidList[position].isChecked) {
    ...
    fun getAllSelected() : ArrayList<GalleryEnt> {
        selectedAllVidList = ArrayList()
        for (his in vidList) {
            if (his.isChecked) {
                selectedAllVidList.add(his)
            }
        }
        return selectedAllVidList

    }

    fun setChecked(position: Int) {
        vidList[position].isChecked = !vidList[position].isChecked
        notifyItemChanged(position)
    }

    fun setData(list: List<GalleryEnt>) {
        vidList = list as ArrayList<GalleryEnt>
        notifyDataSetChanged()
    }
