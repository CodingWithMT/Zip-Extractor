...
import com.example.zipfilemanger.databinding.FragmentWordDocuBinding
import com.example.zipfilemanger.models.CompressDataClass


class WordDocuFragment : Fragment() {

    private var _binding: FragmentWordDocuBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentWordDocuBinding.inflate(inflater, container, false)

        getDocuFiles()

        binding.recycleViewWordDocu.setHasFixedSize(true)
        binding.recycleViewWordDocu.layoutManager = LinearLayoutManager(requireContext())
        compressAdaptor = DocumentAdaptor(requireContext(), documentList)
        binding.recycleViewWordDocu.adapter = compressAdaptor
    ...
    private fun getDocuFiles(){
        ...
        val selectedArgs = arrayOf("application/msword")
