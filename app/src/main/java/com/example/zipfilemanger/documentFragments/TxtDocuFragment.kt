...
import com.example.zipfilemanger.databinding.FragmentTxtDocuBinding
import com.example.zipfilemanger.models.CompressDataClass


class TxtDocuFragment : Fragment() {

    private var _binding: FragmentTxtDocuBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentTxtDocuBinding.inflate(inflater, container, false)

        getDocuFiles()

        binding.recycleViewTxtDocu.setHasFixedSize(true)
        binding.recycleViewTxtDocu.layoutManager = LinearLayoutManager(requireContext())
        compressAdaptor = DocumentAdaptor(requireContext(), documentList)
        binding.recycleViewTxtDocu.adapter = compressAdaptor
    ...
    private fun getDocuFiles(){
        ...
        val selectedArgs = arrayOf("text/plain")
