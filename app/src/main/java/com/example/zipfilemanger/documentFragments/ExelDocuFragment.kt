...
class ExelDocuFragment : Fragment() {

    private var _binding: FragmentExelDocuBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentExelDocuBinding.inflate(inflater, container, false)

        getDocuFiles()

        binding.recycleViewExelDocu.setHasFixedSize(true)
        binding.recycleViewExelDocu.layoutManager = LinearLayoutManager(requireContext())
        compressAdaptor = DocumentAdaptor(requireContext(), documentList)
        binding.recycleViewExelDocu.adapter = compressAdaptor
    ...
    private fun getDocuFiles(){
        ...
        val selectedArgs = arrayOf("application/vnd.ms-excel")
