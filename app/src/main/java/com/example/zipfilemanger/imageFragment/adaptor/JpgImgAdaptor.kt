...
var jpgLongClick : ((Int) -> Unit)? = null
var jpgItemClick : ((Int) -> Unit)? = null

class JpgImgAdaptor(val context: Context, var jpgList: ArrayList<GalleryEnt>) :
    RecyclerView.Adapter<JpgImgAdaptor.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val selectFolder = jpgList[position]
        ...
        holder.itemView.setOnLongClickListener {
            jpgLongClick?.invoke(position)
            true
        }
        holder.itemView.setOnClickListener {
            jpgItemClick?.invoke(position)
        }

        if (jpgList[position].isChecked) {
    ...
    fun getAllSelected() : ArrayList<GalleryEnt> {
        selectedjpgList = ArrayList()
        for (his in jpgList) {
            if (his.isChecked) {
                selectedjpgList.add(his)
            }
        }
        return selectedjpgList

    }

    fun setChecked(position: Int) {
        jpgList[position].isChecked = !jpgList[position].isChecked
        notifyItemChanged(position)
    }

    fun setData(list: List<GalleryEnt>) {
        jpgList = list as ArrayList<GalleryEnt>
        notifyDataSetChanged()
    }
