...
var audioLongClick : ((Int) -> Unit)? = null
var audioItemClick : ((Int) -> Unit)? = null

class AllAudioAdaptor(val context: Context, var audioList: ArrayList<GalleryEnt>) :
    RecyclerView.Adapter<AllAudioAdaptor.ViewHolder>() {

    private var selectedAudioList: ArrayList<GalleryEnt> = ArrayList()
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.itemView.setOnLongClickListener {
            audioLongClick?.invoke(position)
            true
        }
        holder.itemView.setOnClickListener {

            audioItemClick?.invoke(position)
        }

        if (audioList[position].isChecked) {
    ...
    fun getAllSelected() : ArrayList<GalleryEnt> {
        selectedAudioList = ArrayList()
        for (his in audioList) {
            if (his.isChecked) {
                selectedAudioList.add(his)
            }
        }
    ...
    fun setChecked(position: Int) {
        audioList[position].isChecked = !audioList[position].isChecked
