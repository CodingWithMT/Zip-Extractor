...
import com.example.zipfilemanger.models.btnOkBottomSheetAud
import com.example.zipfilemanger.models.cancelBottomSheetAud
...
class AllAudioFragment : Fragment() {
    ...
    private var _binding: FragmentAllAudioBinding? = null
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        _binding = FragmentAllAudioBinding.inflate(inflater, container, false)
        ...
        viewVModelAud.getAllAudio().observe(requireActivity()) {
            audioAdaptor?.setData(it)
        }

        binding.recycleViewAudioAll.setHasFixedSize(true)
        binding.recycleViewAudioAll.layoutManager = LinearLayoutManager(requireContext())
        audioAdaptor = AllAudioAdaptor(requireContext(), audioList)
        binding.recycleViewAudioAll.adapter = audioAdaptor
...
//           compressImage.visibility = View.VISIBLE
        ...
        btnOkBottomSheetAud = {
            val includeBottom = requireActivity().findViewById<LinearLayout>(R.id.include_Bottom_Sheet_Aud)
